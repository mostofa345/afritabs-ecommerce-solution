...
const CartDrawer = ({ isOpen, onClose }) => {
    ...
    return (
        ...
        >
            <div
                className="absolute top-0 right-0 h-full w-full md:w-2/3 lg:w-1/2 bg-white shadow-lg p-6 overflow-y-auto transform transition-transform duration-300 ease-in-out"
                ...
                {cartItems.length === 0 ? (
                    ...
                    <>
                        ...
                        <div className="md:hidden space-y-4 mb-6">
                            {cartItems.map((item) => (
                                <div key={item._id} className="border border-gray-300 rounded p-4 flex flex-col gap-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 border-t pt-4">
                                        <div>
                                            <span className="font-semibold">Unit Price:</span> R{item.product?.price?.toFixed(2)}
                                        </div>
                                        <div>
                                            <span className="font-semibold">Subtotal:</span> R{(item.product?.price * item.quantity).toFixed(2)}
                                        </div>
                                        <div className="col-span-2">
                                            <span className="font-semibold">Quantity:</span>
                                            <QuantityButton
                                                quantity={item.quantity}
                                                onDecrement={() => handleUpdateQuantity(item._id, item.product._id, item.quantity - 1)}
                                                onIncrement={() => handleUpdateQuantity(item._id, item.product._id, item.quantity + 1)}
                        ...
                        <table className="hidden md:table w-full text-left border-collapse border border-gray-300 mb-6">
                            <thead>
                        ...
                        <div className="flex justify-between items-start gap-6 flex-wrap">
                            ...
                            <div className="w-full md:w-1/2 flex items-end">
                                <button
                                    className="bg-green-600 text-white px-4 py-2 mt-4 w-full rounded hover:bg-green-700 transition-colors"
